...
Based on the dataframe, scores have improved because the average score was 72.6, but now every score is above that in the new data set. I can tell because the range of scores is between 73 and 79.
...
The mean increased to 75.75 and the stand deviation decreased to 1.76455, so the observations I can make from this is that the reading scores overall increased by around an average of three points, and now the scores are closer together as the standard deviation is smaller. The total range of scores is only between 73 and 79. My questions would be why are the scores between all students becoming more similar? Also, why did the lower end of the range increase higher than the previous average. 
...
Reading scores based on new program compared to previous average
...
**Hypotheses:**
Null: The new reading program increased the average reading scores
Alternative: The new reading program influenced students getting similar reading scores

```{r}
# Set the significance level for your test
alpha_level <- 75.75 # Replace with the appropriate value
...
The p-value is 3.435e-05 which is less than 0.05 which means that it is statistically significant. Which means that the mean of 75.75 is a significant variable when detecting that the true mean is greater than 72.6 with a 95 percent confidence interval. It doesn't tell you have much the mean increased or how it did, but it tells you that the new mean increasing is statistically significant. 
...
A new reading program has increased average third-grade reading scores

Statistical evidence proved the superintendent's claim that average reading scoreds improved for third graders after the implementation of a new reading program.

Evidence showed that average reading scores improve by over three percent since the new program began. 
...
Student's english grades
amount of books read
Enrollment in reading progran
